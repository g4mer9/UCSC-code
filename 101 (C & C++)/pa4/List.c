...
void deleteFront(List L) {
    ...
    if(L->index == 0) L->cursor = NULL;//making cursor undefined if it's being deleted
    if(L->length == 1) {
        delete_node(&L->front);
    ...
    L->front->prev = NULL;
    L->length--;
    if(L->index != -1) L->index--;//if index == 0, makes undefined, else lowers index since node to left deleted
}
